...
AssociationId's are only availble when a NACL has a subnet associated. If there's no default subnet, the association won't happen after the create-network-acl command.
...
<pre>aws ec2 authorize-security-group-ingress --group-id sg-0f2ccacdf34c96f62 --protocol tcp --port 22 --cidr 10.0.0.0/26</pre>

#### Create egress rule for HTTPS 443 to 0.0.0.0/0 in Private34 security group
<pre>aws ec2 authorize-security-group-egress --group-id sg-0f2ccacdf34c96f62 --protocol tcp --port 443 --cidr 0.0.0.0/0</pre>

#### Create egress rule for ICMP to 0.0.0.0/0 in Private34 security group
<pre>aws ec2 authorize-security-group-egress --group-id sg-0f2ccacdf34c96f62 --ip-permissions IpProtocol=icmp,FromPort=-1,ToPort=-1,IpRanges='[{CidrIp=0.0.0.0/0}]'</pre>
...
<pre>aws ec2 modify-instance-attribute --instance-id i-065de3e8aceb8decb --groups sg-0f2ccacdf34c96f62</pre>
...
aws ec2 create-network-acl --tag-specifications 'ResourceType=network-acl,Tags=[{Key="Name",Value="TOOLS_Public2"}]' --vpc-id $VPC_ID
{
    "NetworkAcl": {
        ...
        "IsDefault": false,
        "NetworkAclId": "acl-0b29c80d13f5cf093",
        "Tags": [
            {
                "Key": "Name",
                "Value": "TOOLS_Public2"
        ...
        "VpcId": "vpc-0ebc617db748f94c9",
        "OwnerId": "<ACCT_ID>"
    }
}
export PUBLIC2_NACL_ID=acl-0b29c80d13f5cf093
aws ec2 create-network-acl-entry --ingress --rule-number 110 --rule-action allow --port-range From=443,To=443 --protocol tcp --cidr-block 10.0.0.0/24 --network-acl-id $PUBLIC2_NACL_ID
...
aws ec2 create-network-acl-entry --egress --rule-number 120 --rule-action allow --port-range From=1024,To=65535 --protocol tcp --cidr-block 10.0.0.0/24 --network-acl-id $PUBLIC2_NACL_ID
...
<pre>aws ec2 create-nat-gateway --allocation-id eipalloc-0ce84b82c135cc7c7 --tag-specifications 'ResourceType=natgateway,Tags=[{Key="Name",Value="TOOLS_Public2_NATGW"}]' --subnet-id subnet-0b5c80bfd5ebc5211</pre>
